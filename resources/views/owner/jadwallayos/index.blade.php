    ...
    <div class="page-title">
        <div class="row align-items-center">
            ...
            <div class="col-12 col-md-5 d-flex justify-content-md-end align-items-center mt-3 mt-md-0 gap-2">
                ...
                <a href="{{ route('owner.jadwallayos.create') }}" class="btn btn-primary shadow-sm px-3 fw-bold">
...
$(document).ready(function () {
    ...
    let table = $('#jadwal-layos-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route(name: 'owner.jadwallayos.data') }}",
            data: function(d) {
    ...
    $('#btn-print').click(function () {
        let url = "{{ route('owner.jadwallayos.print') }}?bulan=" + $('#filter-bulan').val() + "&tahun=" + $('#filter-tahun').val();
...
function hapusJadwal(id){
    Swal.fire({
        ...
        if(result.isConfirmed){
            $.ajax({
                url: "{{ url('owner/jadwallayos/destroy') }}/" + id, 
                type: "DELETE",
