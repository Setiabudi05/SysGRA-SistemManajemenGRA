                        ...
                        @else
                            @foreach($carts as $cart)
                                <div class="p-4 mb-3 border rounded-4 bg-white shadow-xs">
                                    <div class="row align-items-center">
                                        <div class="col-md-7">
                                            <span class="badge bg-light-primary text-primary mb-2 text-uppercase fw-bold"
                                                style="font-size: 0.6rem;">DRAFT</span>
                                            ...
                                            <p class="mb-1 small text-muted"><i
                                                    class="bi bi-calendar-event me-2 text-primary"></i>{{ date('d M Y', strtotime($cart->event_date)) }}
                                            </p>
                                            <p class="mb-3 small text-muted"><i
                                                    class="bi bi-geo-alt me-2 text-primary"></i>{{ $cart->event_address }}</p>
                                        ...
                                        <div class="col-md-5 text-md-end mt-3 mt-md-0">
                                            <div class="mb-2"><small class="text-muted d-block">Harga Paket:</small><span
                                                    class="fw-bold text-dark">Rp
                                                    {{ number_format((int) $cart->paket->harga, 0, ',', '.') }}</span></div>
                                            <div class="mb-2"><small class="text-muted d-block">Harga Tambahan:</small><span
                                                    class="fw-bold text-info">Rp
                                                    {{ number_format($cart->addOns->sum('harga'), 0, ',', '.') }}</span></div>
                                            <div class="border-top pt-2 mt-2"><small
                                                    class="text-muted d-block">Subtotal:</small><span
                                                    class="fw-bold text-primary h5">Rp
                                                    {{ number_format($cart->total_harga, 0, ',', '.') }}</span></div>

                                            <button type="button" class="btn btn-danger btn-sm rounded-pill px-4 shadow-sm mt-3"
                                                onclick="confirmDelete('{{ $cart->id }}')">
                                                <i class="bi bi-trash me-1"></i> Batalkan
                                            </button>
                                            <form id="delete-form-{{ $cart->id }}"
                                                action="{{ route('user.keranjang.destroy', $cart->id) }}" method="POST"
                                                class="d-none">@csrf @method('DELETE')</form>
                                        </div>
                                    </div>
                                </div>

                                {{-- Modal Detail Dikembalikan --}}
                                        ...
                                        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                                            ...
                                            <div class="modal-body p-4">
                                                <table class="table table-borderless">
                                                    <tr>
                                                        <td class="text-muted w-50">Nama Pemesan</td>
                                                        <td class="fw-bold">{{ $cart->customer_name }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted">Nama Pengantin</td>
                                                        <td class="fw-bold">{{ $cart->bride_groom_name }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted">Durasi Acara</td>
                                                        <td class="fw-bold">{{ $cart->event_duration }} Hari</td>
                                                    </tr>
                                                    <tr>
                                                        <td class="text-muted">Catatan Khusus</td>
                                                        <td class="fw-bold">{{ $cart->notes ?? '-' }}</td>
                                                    </tr>
                                                </table>
                                            </div>
                                        </div>
                    ...
                    <div class="card-body p-4 text-start">
                        ...
                        <div class="d-flex justify-content-between mb-4">
                            ...
                            <span class="fw-bold text-primary h5 mb-0">
                                Rp {{ number_format($carts->sum('total_harga'), 0, ',', '.') }}
                            </span>
                        </div>
                        ...
                        <div class="alert alert-light-primary border-0 small mt-4 py-2"
                            style="background-color: #f0f3ff; color: #435ee0; border-radius: 10px;">
